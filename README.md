...
3. Leave Automix on for continuous playback. Choose a **Warm blend** with a gradual bass handoff or a **Clean** equal-power fade. The length slider requests 4–96 seconds; short tracks shorten the overlap automatically.
...
- **Follow musical phrases is on by default**, under Settings → Playback. A lightweight structural analysis compares sustained changes in bass, midrange and treble energy around each beat. It requires at least three recurring changes fitting an unambiguous 8-, 16- or 32-bar pattern, plus agreement between the local beat grids. This assumes steady 4/4 music; it does not simply label the start of every file as a downbeat. The approach draws on the idea of [novelty-based musical segmentation](https://www.audiolabs-erlangen.de/resources/MIR/FMP/C4/C4S4_NoveltySegmentation.html).
- Phrase matches align both the beginning and end of an 8-, 16- or 32-bar overlap to the two estimated phrase grids. The planner stays within analyzed steady regions, skips at most 24 seconds of the incoming intro, and trims no more than eight bars or 18% of the outgoing playback span, whichever is shorter. Longer blends need a higher length setting and sufficient audio on both sides. It prefers less busy midrange/treble overlaps; if both sections remain busy, it uses a shorter beat blend. Energy is only a proxy for activity, **not vocal detection**.
- **PHRASE MATCHED**, the bar count and gold waveform markers expose the estimate. Listening Mode also shows the scheduled phrase blend length. Uncertain structure or a blend too short for complete phrases falls back to ordinary beat matching; unreliable beat grids retain the gentle crossfade. These are estimates, not guaranteed recognition of every musical section.
- To correct a track, open **Settings → Phrase cue for this track**, enter a known phrase start in seconds (or use the current position), choose its phrase length, and save. The cue snaps to the nearest detected beat and repeats at that interval. It applies to future scheduled transitions, is saved on this device for the same track ID, and can be cleared with **Use automatic**. Saving a cue does not bypass tempo and overlap safety limits. When automatic structure is unavailable, manual cues use neutral activity rather than claiming to detect quieter sections.
- Speed changes are limited to **±6%** using native AudioBuffer playback rate. This is vinyl-style matching: **pitch changes with speed**, and the matched speed stays constant for the track to prevent drift during the blend. There is no pitch lock, key detection, or stem separation. Disable Sync beats during automix in Settings for original-speed future tracks.
